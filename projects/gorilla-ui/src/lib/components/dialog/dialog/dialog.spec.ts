...
import { Spectator, createComponentFactory } from '@guiDialog/spectator';
...
describe('Dialog', () => {
  ...
  afterEach(() => {
    const dialogEls = document.querySelectorAll('guiDialog-dialog');
      ...
      describe(method, () => {
        describe(`should open it`, () => {
          it('using a ng-template', () => {
            ...
            expect(spectator.query(`guiDialog-dialog-${method}`, { root: true })).toBeTruthy();
        ...
        it('should close it', () => {
          ...
          expect(spectator.query(`guiDialog-dialog-${method}`, { root: true })).toBeFalsy();
        });
      });
    });
